...
const StudentDetailPage = () => {
  ...
  const fetchStudentDetail = async () => {
    ...
    try {
      ...
      form.setFieldsValue({
        ...
        teachers: response.data.teachers?.map((teacher) => teacher.id) || [],
      });
    } catch (error) {
      console.error("Error fetching student details:", error);
      message.error("학생 정보를 불러오는데 실패했습니다");
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchDiaries = async () => {
    ...
    try {
      const response = await getStudentDiaryList(id, diaryPage, 10);
      ...
      message.error("학생일지를 불러오는데 실패했습니다");
    } finally {
  ...
  const handleEdit = async (values) => {
    try {
      ...
      if (values.teachers?.length) {
        formattedValues.teachers = values.teachers.map((teacherId) => ({
          id: teacherId,
        }));
      }
      if (values.birthDate) {
      ...
      await updateStudent(id, formattedValues);
  ...
  const handleDiarySubmit = async (values) => {
    try {
      if (selectedDiary) {
        await updateStudentDiary(selectedDiary.id, {
          ...values,
          date: values.date ? values.date.format("YYYY-MM-DD") : null,
        });
        message.success("학생일지가 수정되었습니다.");
      } else {
        await createStudentDiary({
          ...values,
          studentId: id,
          date: values.date ? values.date.format("YYYY-MM-DD") : null,
        });
        message.success("학생일지가 등록되었습니다.");
      }
      ...
      message.error("학생일지 저장 중 오류가 발생했습니다.");
    }
  };
  ...
  const handleDeleteDiary = async (diary) => {
    try {
      AntModal.confirm({
        ...
        cancelText: "취소",
        onOk: async () => {
          await deleteStudentDiary(diary.id);
          message.success("학생일지가 삭제되었습니다.");
          fetchDiaries();
        },
      });
    } catch (error) {
  ...
  const diaryColumns = [
    ...
    },
    {
      ...
      key: "teacher",
      width: 120,
      render: (_, record) => <TeacherTag teacherId={record.teacherId} />,
    },
    {
      title: "관리",
      key: "action",
    ...
    >
      ...
      <Form.Item style={{ marginBottom: 0 }}>
        ...
        >
          ...
          <Space>
            <Button onClick={() => navigate("/student")}>취소</Button>
  ...
  const fetchPayments = async () => {
    ...
    try {
      const response = await getStudentPaymentDetail(
        id,
        Math.max(
          1,
  ...
  return (
    <div style={{ padding: isMobile ? "16px" : "24px" }}>
      ...
      >
        <TabPane tab="학생 정보" key="1">
          {renderStudentForm()}
        </TabPane>
        <TabPane tab="학생일지" key="2">
          <div>
                      ...
                      <List.Item.Meta
                        ...
                        description={
                          <Space direction="vertical" size="small">
                            ...
                            <TeacherTag teacherId={diary.teacherId} />
                          </Space>
                        }
                      />
                    </List.Item>
      ...
      <AntModal
        title={selectedDiary ? "학생일지 수정" : "학생일지 작성"}
