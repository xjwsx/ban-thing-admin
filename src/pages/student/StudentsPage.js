...
import {
  ...
  updateStudent,
  createStudent,
  getStudentsByRegistrationMonth,
  getStudentsByRegistrationYear,
  deleteStudent,
  getTeacherList,
  getCourseList,
} from "../../api/crm";
...
const ManageStudentPage = () => {
  ...
  const [yearlyStudents, setYearlyStudents] = useState([]);
  ...
  const [filters, setFilters] = useState(() => {
    return {
      ...
      status: searchParams.get("status") || "수강중",
  ...
  const fetchYearlyStudents = async (year = dayjs().format("YYYY")) => {
    try {
      const response = await getStudentsByRegistrationYear(year);
      setYearlyStudents(response.data);
    } catch (error) {
      console.error("Error fetching yearly students:", error);
      message.error("연도별 학생 목록을 불러오는데 실패했습니다");
    }
  };
  ...
  const renderActiveFilters = () => {
    ...
    if (filters.teacherId) {
      ...
      activeFilters.push(
        <Tag key="teacherId" closable onClose={() => clearFilter("teacherId")}>
          담당 선생님: {teacher?.name || filters.teacherId}
        </Tag>
      );
    }
  ...
  // 수정: 월 셀 렌더링 함수를 업데이트하여 학생 이름 목록도 함께 표시 (불필요한 내부 스크롤 제거)
  const monthCellRender = (value) => {
    const year = value.format("YYYY");
    const month = value.format("MM");
    const studentsInMonth = yearlyStudents.filter(
      (student) =>
        dayjs(student.registrationDate).format("YYYY-MM") === `${year}-${month}`
    );
    if (studentsInMonth.length === 0) return null;
    return (
      <div
        className="notes-month"
        style={{ padding: "4px" }} // 내부 스크롤 관련 스타일 제거하여 외부 스크롤만 사용
      >
        <div style={{ fontWeight: "bold", marginBottom: "4px" }}>
          {month}월: {studentsInMonth.length}명
        </div>
        <ul style={{ margin: 0, padding: 0, listStyle: "none" }}>
          {studentsInMonth.map((student) => (
            <li key={student.id} style={{ whiteSpace: "nowrap" }}>
              {student.name}
            </li>
          ))}
        </ul>
  ...
  const getListData = (value) => {
    ...
    return calendarStudents
      .filter(
        (student) =>
          dayjs(student.registrationDate).format("YYYY-MM-DD") === dateStr
      )
      .map((student) => ({
        type: "success",
  ...
  const handleCalendarChange = (date, mode) => {
    if (mode === "year") {
      fetchYearlyStudents(date.format("YYYY"));
    } else {
      fetchCalendarStudents(date);
    }
  };

  const fetchCalendarStudents = async (date) => {
    try {
      const year = date.format("YYYY");
      const month = date.format("MM");
      const response = await getStudentsByRegistrationMonth(year, month);
      setCalendarStudents(response.data);
    } catch (error) {
      console.error("Error fetching calendar students:", error);
      ...
      render: (teachers) => (
        <Space>
          {teachers?.map((teacher) => (
            <TeacherTag key={teacher.id} teacher={teacher} />
          ))}
        </Space>
      ),
    },
  ...
  const handleDeleteStudent = async (student) => {
    try {
      Modal.confirm({
        title: "학생 삭제",
        content: `${student.name} 학생을 삭제하시겠습니까?`,
        okText: "삭제",
        okType: "danger",
        cancelText: "취소",
        onOk: async () => {
  ...
  const fetchTeachers = async () => {
    try {
      const response = await getTeacherList(1, 50);
  ...
  const fetchStudents = async (
    ...
    setLoading(true);
    try {
      const response = await getStudentList(currentPage, PAGE_SIZE, {
        ...currentFilters,
      ...
      setMetadata({
        ...
        totalPage: Math.ceil(response.data.total / PAGE_SIZE),
      });
    } catch (error) {
  ...
  const showEditModal = (student) => {
    ...
    form.setFieldsValue({
      ...student,
      registrationDate: dayjs(student.registrationDate).isValid()
        ? dayjs(student.registrationDate)
        : null,
      birthDate: dayjs(student.birthDate).isValid()
        ? dayjs(student.birthDate)
        : null,
      deadline: dayjs(student.deadline).isValid()
        ? dayjs(student.deadline)
        : null,
      experienceDate: dayjs(student.experienceDate).isValid()
        ? dayjs(student.experienceDate)
        : null,
      hopeDateText: student.hopeDateText,
    });
  ...
  const handleEdit = async (values) => {
    try {
      await updateStudent(selectedStudent.id, formatDateValues(values));
      message.success("학생 정보가 수정되었습니다.");
      setEditModalVisible(false);
      fetchStudents();
      fetchCalendarStudents(dayjs());
    } catch (error) {
  ...
  const handleRegister = async (values) => {
    try {
      const formattedValues = {
        ...formatDateValues(values),
        teachers:
          values.teachers?.map((teacherId) => ({ id: teacherId })) || [],
  ...
  // Form rendering
  const renderStudentForm = (formInstance, onFinish, isEdit = false) => (
    <Form
      form={formInstance}
      layout="vertical"
      onFinish={onFinish}
    ...
    >
      ...
      >
        ...
        <Row gutter={16}>
          ...
          <Col span={8}>
            <Form.Item name="teachers" label="담당 선생님">
              <Select mode="multiple" placeholder="담당 선생님을 선택하세요">
                {teachers.map((teacher) => (
  ...
  return (
    <div style={{ padding: isMobile ? "0" : "0 0 12px 0" }}>
      ...
      {!isMobile && (
        ...
        >
          <Form form={searchForm} onFinish={handleSearch} layout="inline">
            ...
            <Form.Item name="teacherId" style={{ marginBottom: 8 }}>
              <Select
                placeholder="담당 선생님"
                style={{ width: 120 }}
                allowClear
      ...
      <Tabs
        ...
        onChange={(key) => {
          ...
          if (key === "2") {
            ...
            fetchCalendarStudents(currentDate);
            fetchYearlyStudents(currentDate.format("YYYY"));
          }
        }}
        items={[
          {
                          ...
                          <List.Item.Meta
                            title={
                              <Space>
                                ...
                                <Tag
                                  color={
                                    STATUS_COLORS[student.status] || "default"
                                  }
                                >
                                  {student.status}
                                </Tag>
      ...
      <Modal
        ...
        style={{ top: 20 }}
      >
        {renderStudentForm(registerForm, handleRegister)}
      </Modal>
      ...
      <Modal
        ...
        style={{ top: 20 }}
      >
        {renderStudentForm(form, handleEdit, true)}
      </Modal>
    </div>
  );
};
